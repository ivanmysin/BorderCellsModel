...
def _train_stateful(model, X, Y, n_batches, n_epochs, batches_per_epoch,
    ...
    rnn_layer = model.get_layer("border_rnn").cell
    max_start = max(0, n_batches - batches_per_epoch)

    model.compile(
        optimizer=tf.keras.optimizers.Adam(
            learning_rate=model.optimizer.learning_rate, clipvalue=10.0),
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % log_every == 0 or epoch == n_epochs - 1:
            tag = f"phase2_epoch_{epoch+1:04d}_loss_{avg:.6f}"
            ...
            with open(os.path.join(checkpoint_dir, f"{tag}_meta.json"), "w") as f:
                json.dump({"epoch": epoch + 1, "loss": avg}, f, indent=2)
    return loss_history
